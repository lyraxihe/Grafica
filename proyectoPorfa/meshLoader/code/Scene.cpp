...
namespace udit
{
    ...
    const string Scene::texture_path = "../../../shared/assets/florT.jpeg";
        ...
        
    {
       ...
       // glEnable     (GL_CULL_FACE);
        ...
        // Cargar la textura usando la función que acabamos de definir:
        texture_id = create_texture_2d(texture_path);
        there_is_texture = texture_id > 0;

        GLuint textureUniformLocation = glGetUniformLocation(program_id, "sampler");
    ...
    void Scene::load_mesh(const std::string& mesh_file_path)
    {
        ...
        if (scene && scene->mNumMeshes > 0)
        {
            ...
            // Subir las coordenadas de textura (UV)
    ...
    GLuint Scene::create_texture_2d(const std::string& texture_path)
    {
        ...
        uint8_t* loaded_pixels = SOIL_load_image(
            ...
            &image_height,
            &image_channels,
            SOIL_LOAD_RGBA  // Forzamos que la imagen se cargue en formato RGBA
        );

        if (loaded_pixels)
        {
            // Habilitar y generar la textura en OpenGL
            GLuint texture_id;
